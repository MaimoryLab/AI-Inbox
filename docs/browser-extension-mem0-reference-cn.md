...
| 页面诊断 | `diagnostics` + “复制问题信息” | AI 网页 DOM 经常变，逐站适配必须可反馈、可复现 |
...
| 同步侧栏 | `browser-extension/sidepanel.*` | 展示上下文、候选草稿、复制问题信息、打开测试卡 | 真实站点失败时能直接复制可复现诊断 |
...
4. 每次真实网页失配时，用侧栏“复制问题信息”补证据，再更新 selector 和验收记录。
