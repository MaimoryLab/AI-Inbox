...
// existing. Fire-and-forget trigger keeps the HTTP response fast (kv.update
...
describe("api::session::end → event::session::stopped (#666)", () => {
  ...
  it("api::session::end calls sdk.trigger('event::session::stopped') after kv.update", () => {
    expect(api).toMatch(
      /api::session::end[\s\S]*?kv\.update\(KV\.sessions[\s\S]*?trigger\(\{\s*function_id:\s*"event::session::stopped"/,
    );
  });

  it("session::stopped trigger payload includes sessionId", () => {
    expect(api).toMatch(
      /trigger\(\{\s*function_id:\s*"event::session::stopped",\s*payload:\s*\{\s*sessionId\s*\}/,
    );
  });
});
