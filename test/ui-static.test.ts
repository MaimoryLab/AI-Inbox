...
test("React UI keeps card-first Sources wording and accessible icon controls", () => {
  ...
  assert.match(i18n, /llmProtocol: "协议"/);
  assert.match(i18n, /llmProtocol: "API protocol"/);
  assert.match(i18n, /testExtractionConfig: "测试抽取配置"/);
  assert.match(i18n, /testExtractionConfig: "Test extraction config"/);
  assert.match(settingsWorkspace, /text\.llmProtocol/);
  assert.match(settingsWorkspace, /text\.testExtractionConfig/);
  assert.match(settingsWorkspace, /preflightCheckText\(check, locale\)/);
  assert.match(settingsWorkspace, /preflightCheckClass\(check\.status\)/);
  assert.match(settingsWorkspace, /text-emerald-700/);
  assert.match(settingsWorkspace, /text-red-700/);
  assert.match(app, /preflightCheckText\(check, locale\)/);
  assert.doesNotMatch(settingsWorkspace, /\$\{check\.status\} \$\{check\.id\}/);
  assert.match(settingsWorkspace, /openai-chat/);
  assert.match(settingsWorkspace, /openai-responses/);
  assert.match(settingsWorkspace, /anthropic-messages/);
  ...
  assert.doesNotMatch(client, /sessionStorage\.getItem\("ai-inbox-token"\)/);
  ...
  assert.doesNotMatch(client, /x-ai-inbox-token/);
