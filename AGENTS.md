...
AI-Index is a local-first todo extraction product for AI/browser/agent sessions.
...
- Local data directories: `data/`, `data-*`, `.local/`, `.ai-index/`.
...
`ai-index open` defaults to `127.0.0.1:3111`. Keep `--port <n>` as the explicit
...
`ai-index open --port <port>`; do not silently choose a random port.
...
For local UI verification, build first, start `ai-index open`, and inspect
